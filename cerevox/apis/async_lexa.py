"""
Cerevox SDK's Asynchronous Lexa Client for Document Processing
"""
...
class AsyncLexa(AsyncIngest):
    """
    Official Asynchronous Python Client for Cerevox Lexa Document Processing.
    ...
    Examples
    --------
    Basic document processing with context manager (recommended):

    >>> async with AsyncLexa(api_key="your_token") as client:
    ...     # Process local files
    ...     documents = await client.parse(["report.pdf", "data.xlsx"])
    ...
    ...     ]
    ...     documents = await client.parse_urls(urls)
    ...
    Advanced processing with custom callbacks:

    >>> def progress_handler(status):
    ...     print(f"Progress: {status.progress}% - {status.completed_files}/{status.total_files} files")
    ...
    >>> async with AsyncLexa(api_key="your_token") as client:
    ...     documents = await client.parse(
    ...         files=["large_document.pdf"],
    ...         progress_callback=progress_handler,
    ...         max_poll_time=1200.0  # 20 minutes for large files
    ...     )

    Notes
    -----
    This client is designed for async/await usage and should be used within
    an async context manager to ensure proper resource cleanup and session
    management.
    ...
    def __init__(
        ...
        *,
        ...
        data_url: Optional[str] = "https://www.data.cerevox.ai",
        auth_url: Optional[str] = "https://dev.cerevox.ai/v1",
        ...
        **kwargs: Any,
        ...
        """
        Initialize the AsyncLexa client for document processing operations.
        ...
        max_concurrent : int, default 10
            Maximum concurrent requests for batch operations. Values between
            5-20 typically provide optimal performance. Higher values may
        ...
        poll_interval : float, default 2.0
            Time in seconds between job status polling attempts. Shorter
            intervals provide faster completion detection but increase API load.
        ...
        **kwargs : dict
            ...
            connector, trust_env, proxy, ssl, or cookie settings.
        ...
        A ThreadPoolExecutor is created for handling blocking operations
        during document processing workflows. The executor is automatically
        ...
        """
        # Validate data_url specifically for explicit None or empty string
        ...
        # Initialize the async ingest service with dual URL support
    ...
    async def __aenter__(self) -> "AsyncLexa":
        ...
        -------
        AsyncLexa
            The initialized client instance ready for document processing.

        Notes
        -----
        Automatically initializes the HTTP session and authentication.
        Always use the client within an async context manager to ensure
    ...
    async def __aexit__(
        ...
        Notes
        -----
        Automatically closes HTTP sessions and thread pools to prevent
        resource leaks. Called automatically when exiting the context manager.
        """
    ...
    async def close_session(self) -> None:
        ...
        Notes
        -----
        Gracefully closes the aiohttp session and shuts down the thread
        executor. This method is called automatically by the context manager
    ...
    async def parse(
        ...
        >>> documents = await client.parse("report.pdf")
        >>> doc = documents[0]
        ...
        # Validate that we got a valid request_id from upload
        if not result.request_id or result.request_id.strip() == "":
    ...
    async def parse_urls(
        ...
        ... ]
        >>> documents = await client.parse_urls(urls, mode=ProcessingMode.ENHANCED)
    ...
    async def parse_s3_folder(
        ...
        IAM roles, or AWS configuration files. The service needs permissions
        to list and read objects from the specified S3 location.
    ...
    async def parse_box_folder(
        ...
        >>> documents = await client.parse_box_folder(
    ...
    async def parse_dropbox_folder(
        ...
        >>> documents = await client.parse_dropbox_folder(
    ...
    async def parse_sharepoint_folder(
        ...
        >>> documents = await client.parse_sharepoint_folder(
    ...
    async def parse_salesforce_folder(
        ...
        >>> documents = await client.parse_salesforce_folder(
    ...
    async def parse_sendme_files(
        ...
        >>> documents = await client.parse_sendme_files(
        ...     ticket="abc123xyz789",
        ...     mode=ProcessingMode.FAST
        ... )
