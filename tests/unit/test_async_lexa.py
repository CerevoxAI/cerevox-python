...
class TestGetDocuments:
        ...
        async with AsyncLexa(api_key="test-key") as client:
            with aioresponses.aioresponses() as m:
                ...
                with patch.object(DocumentBatch, "from_api_response") as mock_from_api:
                    ...
                    result = await client._get_documents("test-request-id", None, None, None, True)
    ...
    def test_get_documents_new_format(self):
        ...
        with patch.object(client, '_wait_for_completion', return_value=mock_status_dict):
            with patch('cerevox.async_lexa.DocumentBatch') as MockDocumentBatch:
                mock_batch = Mock()
                MockDocumentBatch.from_api_response.return_value = mock_batch
        ...
        with patch.object(client, '_wait_for_completion', return_value=mock_status_empty):
            with patch('cerevox.async_lexa.DocumentBatch') as MockDocumentBatch:
        ...
        with patch.object(client, '_wait_for_completion', return_value=mock_status_none):
            with patch('cerevox.async_lexa.DocumentBatch') as MockDocumentBatch:
                mock_empty_batch = Mock()
                MockDocumentBatch.return_value = mock_empty_batch
        ...
        mock_file_with_data.data = [
            {
                ...
                "source": {
                    "file": {"name": "valid.pdf", "extension": "pdf", "id": "f1", "index": 0, "mime_type": "application/pdf", "original_mime_type": "application/pdf"},
                    "page": {"page_number": 1, "index": 0},
        ...
        with patch.object(client, '_wait_for_completion', return_value=mock_status_mixed):
            with patch('cerevox.async_lexa.DocumentBatch') as MockDocumentBatch:
                mock_batch = Mock()
                MockDocumentBatch.from_api_response.return_value = mock_batch
        ...
        with patch.object(client, '_wait_for_completion', return_value=mock_status_old):
            with patch('cerevox.async_lexa.DocumentBatch') as MockDocumentBatch:
                ...
                MockDocumentBatch.from_api_response.assert_called_once_with({"test": "old format data"})
                assert result == mock_batch
        ...
        with patch.object(client, '_wait_for_completion', return_value=mock_status_no_data):
            with patch('cerevox.async_lexa.DocumentBatch') as MockDocumentBatch:
                mock_empty_batch = Mock()
                MockDocumentBatch.return_value = mock_empty_batch
...
class TestAsyncLexaNewFormat:
    ...
    def test_create_progress_callback_completion_statuses(self):
        ...
        completion_statuses = [JobStatus.COMPLETE, JobStatus.PARTIAL_SUCCESS, JobStatus.FAILED]
